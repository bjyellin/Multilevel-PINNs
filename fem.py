...
# print("Look here!")
x_vals = np.linspace(-1,1,(nx+1)**2)
y_vals = np.linspace(-1,1,(ny+1)**2)

print(u(0.,0.))

#I should change this to be nx3 so that I can keep track of which input each output I use in training came from
#Then I can compute the loss with the neural network prediction
u_fem_np = np.zeros((mesh.num_vertices(),3))
for i in range(mesh.num_vertices()):
    # print(x_vals[i])
    # breakpoint()
    u_fem_np[i] = data_points[i,:]#np.array([x_vals[i],y_vals[i],data_points[:,-1].reshape(-1,1).squeeze()[i]])
    # breakpoint()
