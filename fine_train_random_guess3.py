...
def run_random_fine(problem, num_interpolation_points, data_source, num_epochs, physics_weight, boundary_weight, data_weight):
    ...
    interior_mask = ~((all_points[:, 0] == 1) | (all_points[:, 0] == -1) | 
                    (all_points[:, 1] == 1) | (all_points[:, 1] == -1))

    interior_points = all_points[interior_mask].clone().detach().requires_grad_() 

    #Define a coarse set of points where we'll calculate the data fit loss (THIS IS WHAT COARSE DATA FIT SHOULD BE CALCULATED ON)
    x_vals_for_coarse_points = torch.linspace(-1, 1, num_interpolation_points)
    y_vals_for_coarse_points = torch.linspace(-1, 1, num_interpolation_points)
    coarse_points_x, coarse_points_y = torch.meshgrid(x_vals_for_coarse_points, y_vals_for_coarse_points, indexing='ij')
    coarse_points_x, coarse_points_y = coarse_points_x.flatten().reshape(-1,1), coarse_points_y.flatten().reshape(-1,1)

    #Define a fine set of points where we'll calculate the physics loss
    x_vals_for_fine_points = torch.linspace(-1, 1, 3*num_interpolation_points)
    y_vals_for_fine_points = torch.linspace(-1, 1, 3*num_interpolation_points)
    fine_points_x, fine_points_y = torch.meshgrid(x_vals_for_fine_points, y_vals_for_fine_points, indexing='ij')
    fine_points_x, fine_points_y = fine_points_x.flatten().reshape(-1,1), fine_points_y.flatten().reshape(-1,1) 

    #Define a very fine set of points to compute the residual of the equations on
    x_super_fine = torch.arange(-1,1.1,1/(5*num_interpolation_points)) #Size 105
    y_super_fine = torch.arange(-1,1.1,1/(5*num_interpolation_points))
    ...
    if problem == 'adr':
        ...
        def generate_boundary(num_points):
            # Generate equally spaced points along each edge
            edges = [
                np.linspace([0, 0], [1, 0], num_points // 4),  # bottom edge
                np.linspace([1, 0], [1, 1], num_points // 4),   # right edge
                np.linspace([1, 1], [0, 1], num_points // 4),   # top edge
                np.linspace([0, 1], [0, 0], num_points // 4)   # left edge
            ]
    ...
    # Create boundary_points by filtering
    boundary_points = torch.tensor(generate_boundary(800))
    # Create the interior mask
    interior_mask_super_fine = ~((all_points_super_fine[:, 0] == domain_x[0]) | 
                                (all_points_super_fine[:, 0] == domain_x[1]) | 
                                (all_points_super_fine[:, 1] == domain_y[0]) | 
                                (all_points_super_fine[:, 1] == domain_y[1]))
    
    #Extract interior points and create a new tensor 
    interior_points_super_fine = torch.tensor(all_points_super_fine[interior_mask_super_fine].detach().numpy(), requires_grad=True)
    ...
    a, b = (data_source==0, data_source==1)
    ...
    def closure():
        ...
        if problem == 'discontinuous':
            ...
            for i in range(len(fine_points_x)):
                fem_interpolated[i,:]=torch.tensor([fine_points_x[i],fine_points_y[i],u(fine_points_x[i],fine_points_y[i])])
        
        if problem == 'adr':
            fem_interpolated = torch.zeros(fine_points_x.shape[0],3)
            adr_data_csv = pd.read_csv('gals_solution.csv')
            adr_data_numpy = adr_data_csv.values
            adr_data_numpy = adr_data_numpy[:,1:]
            # breakpoint()
            adr_gals_tensor = torch.tensor(adr_data_numpy, dtype=torch.float64)
            fem_interpolated = adr_gals_tensor
            # for i in range(len(fine_points_x)):
            #     fem_interpolated = adr_gals_tensor#torch.tensor([fine_points_x[i],fine_points_y[i],u(fine_points_x[i],fine_points_y[i])])
            # breakpoint()
            # u(x_test,y_test)
        ...
        if sampling_fem_data:
            ...
            # breakpoint()
            pinn_interpolated = torch.zeros(fem_interpolated[:,0].shape[0],3)
            # breakpoint()
            model_on_mesh = model(torch.cat((fem_interpolated[:,0].reshape(-1,1).to(torch.float64),fem_interpolated[:,1].reshape(-1,1).to(torch.float64)),dim=1))
            for i in range(len(fem_interpolated[:,0])):
                # breakpoint()
                pinn_interpolated[i,:] = torch.tensor([fem_interpolated[i,0].item(),fem_interpolated[i,1].item(),model_on_mesh[i].item()]) 
        ...
        if (problem == 'nonlinear' or problem == 'discontinuous') and sampling_true_data:
            ...
            #Compute components of data fit loss on interior_points (these are coarse)
            #Generate noise to add to samples of true solution 
            noise=np.random.normal(0,1,size=coarse_points_x.shape).reshape(-1,1)
            noise_tensor = torch.from_numpy(noise).to(torch.float64)
            # breakpoint()
            samples_u = u_true(coarse_points_x,coarse_points_y,problem)+noise_tensor
            
            pinn_pred = model(torch.cat((coarse_points_x,coarse_points_y), dim=1))
            data_fit_interior_loss_true_sol = criterion(samples_u, pinn_pred)
            # breakpoint()
            #Compute interior loss (defined on super fine interior data)
        ...
        if problem == 'adr':
            # print("inside of if problem==adr: ")
            samples_f = None
        ...
        # breakpoint()
        if problem == 'discontinuous' or problem == 'nonlinear':
            boundary_loss = dirichlet_boundary_loss_func(boundary_points[:,0].reshape(-1,1),boundary_points[:,1].reshape(-1,1), model, problem)
        # breakpoint()

        if problem == 'adr':
            #Try to impose 0 dirichlet boundary conditions on the bottom 
            #and left (inflow) and 1 on the top and right (outflow)
            
            boundary_loss = dirichlet_boundary_loss_func(x,y,model,problem)#torch.norm(torch.abs(model(boundary_points)))



            # breakpoint()
        if problem == 'discontinuous' and sampling_true_data: 
            total_loss = physics_weight*residual + boundary_weight*boundary_loss + data_weight*sampling_true_data*data_fit_interior_loss_true_sol 
        
        if problem == 'discontinuous' and not sampling_true_data and sampling_fem_data: 
            total_loss = physics_weight*residual + boundary_weight*boundary_loss + sampling_fem_data*data_fit_interior_loss_fem 

        if problem == 'discontinuous' and not sampling_true_data and not sampling_fem_data:
        ...
        if problem == 'adr':
            ...
            #Unweighted
            total_loss = residual + boundary_loss + data_fit_interior_loss_fem
            # breakpoint()
            if epoch % 10 == 0: 
                print("total loss: ", total_loss)
                print("Residual: ", residual)
                print("boundary loss: ", boundary_loss) #This isn't decreasing right now
                print("data fit loss: ", data_fit_interior_loss_fem)
            #Weighted
            # total_loss = physics_weight*residual + boundary_weight*boundary_loss + data_fit_interior_loss_fem
    ...
    adam_lbfgs_cutoff = num_epochs#int(3*num_epochs/4)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch < adam_lbfgs_cutoff:
            ...
            # optimizer =  torch.optim.LBFGS(model.parameters(),max_iter=1000,line_search_fn='strong_wolfe')
        if epoch >= adam_lbfgs_cutoff:
            optimizer =  torch.optim.LBFGS(model.parameters(),max_iter=1, line_search_fn='strong_wolfe')
    ...
    # img1 = ax1.imshow(fem_interpolated_reshaped, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
    # ax1.set_xlabel('x')
    # ax1.set_ylabel('y')
    ...
    # img2 = ax2.imshow(model_on_mesh_reshaped, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
    # ax2.set_xlabel('x')
    # ax2.set_ylabel('y')
    ...
    # img3 = ax3.imshow(fem_interpolated_reshaped - model_on_mesh_reshaped, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
    # ax3.set_xlabel('x')
    # ax3.set_ylabel('y')
    ...
    fem_data_x = fem_data_tensor[:, 0].unsqueeze(dim=1).to(torch.float64)
    fem_data_y = fem_data_tensor[:, 1].unsqueeze(dim=1).to(torch.float64)
    ...
    if problem == 'discontinuous':
        ...
        cax1 = ax1.imshow(fem_interpolated_reshaped, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        cax2 = ax2.imshow(model_on_mesh_reshaped, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        if sampling_fem_data and sampling_true_data:
            ...
            cax3 = ax3.imshow(fem_interpolated_reshaped-model_on_mesh_reshaped, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
            ...
            cax4 = ax4.imshow(fem_interpolated_reshaped-np.asarray(true_sol), extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
            ...
            # breakpoint()
            cax5 = ax5.imshow(model_on_mesh_reshaped-np.asarray(true_sol), extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        fig4, ax4 = plt.subplots(figsize=(6, 6))
        cax4 = ax4.imshow(true_solution, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        plt.clf()
        fig5, ax5 = plt.subplots(figsize=(6, 6))
        cax5 = ax5.imshow(fem_interpolated_reshaped-np.asarray(true_solution), extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        # f.set_figwidth(5)
        # f.set_figheight(5)
        cax6 = ax6.imshow(np.array(model_on_mesh_reshaped)-np.array(true_solution), extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        # plt.imshow(true_solution, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        # plt.imshow(fem_interpolated_reshaped, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        #     plt.imshow(model_on_mesh_reshaped, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        # plt.subplot(1, 6, 3)
        # plt.title("FEM - PINN")
        # plt.imshow(fem_interpolated_reshaped-model_on_mesh_reshaped, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        # plt.imshow(np.array(fem_interpolated_reshaped)-np.array(true_solution), extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
        ...
        # plt.imshow(np.array(model_on_mesh_reshaped)-np.array(true_solution), extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
    ...
    if problem == 'adr':
        plt.imshow(model_on_mesh_reshaped, extent=(0,1,0,1), origin='lower', cmap='viridis')
    ...
    # plt.imshow(relative_error, extent=(domain_x[0], domain_x[1], domain_y[0], domain_y[1]), origin='lower', cmap='viridis')
