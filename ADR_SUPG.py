...
a = mu*dot(nabla_grad(u), nabla_grad(v))*dx + dot(beta,nabla_grad(u))*v*dx+sigma*u*v*dx + \
    tau*(-mu*div(nabla_grad(u))+dot(beta,nabla_grad(u))+sigma*u)*(dot(beta,nabla_grad(v)))*dx
L = f*v*dx + tau*f*(dot(beta,nabla_grad(v)))*dx
...
vtkfile = File('adrSUPG/solution.pvd')
vtkfile << u
